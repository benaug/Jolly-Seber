...
GetKern <- nimbleFunction(
  run = function(s=double(1),X=double(2), sigma = double(0), z=double(0), z.super=double(0)){ 
    returnType(double(1))
    J=nimDim(X)[1]
...
GetDetectionProb <- nimbleFunction(
  run = function(p0=double(0), kern=double(1), z=double(0), z.super=double(0)){ 
    returnType(double(1))
    if(z.super==0 | z.super==1&z==0){
      J <- nimDim(kern)[1]
...
dBinomialVector <- nimbleFunction(
  run = function(x = double(1), pd=double(1), K=double(1), z = double(0),z.super = double(0),log = integer(0)) {
    returnType(double(0))
    if(z.super==0 | z.super==1&z==0){#skip calculation if not is superpop, or in superpop, but not alive in this year
      ...
      logProb <- 0
      for(j in 1:J){
        logProb <- logProb+dbinom(x[j],prob=pd[j],size=K[j],log=TRUE)
      }
      return(logProb)
    }
  }
  ...
  run = function() {
    ...
    for(i in 1:M){
      if(z.obs[i]==1&y2D[i,1]==0){ #for detected guys, skip if observed 1st year
        ...
        for(g in 1:first.det){ #must be recruited in year with first detection or before
          ...
          model$calculate(ER.nodes) # update ER when N changes
          model$calculate(kern.nodes[i.idx]) #update kern nodes when z changes when z changes
          model$calculate(pd1.nodes[i.idx]) #update kern nodes when pd changes when z changes
          model$calculate(pd2.nodes[i.idx]) #update kern nodes when pd changes when z changes
          ...
          lp.y1 <- model$calculate(y1.nodes[i.idx])
          lp.y2 <- model$calculate(y2.nodes[i.idx])
          lp.surv <- model$calculate(z.nodes[i])
          lp.start[g] <- lp.recruit + lp.y1 + lp.y2 + lp.surv
        }
        ...
        if(model$z.start[i]!=z.start.prop){#if proposal is same as current, no need to replace anything
          ...
          model$calculate(kern.nodes[i.idx]) #update kern nodes when z changes
          model$calculate(pd1.nodes[i.idx]) #update kern nodes when pd changes
          model$calculate(pd2.nodes[i.idx]) #update kern nodes when pd changes
          #update these logProbs
          model$calculate(y1.nodes[i.idx])
          model$calculate(y2.nodes[i.idx])
          ...
          for(g in 1:n.year){
            for(j in 1:J[g]){
              mvSaved["kern",1][i,g,j] <<- model[["kern"]][i,g,j]
              mvSaved["pd1",1][i,g,j] <<- model[["pd1"]][i,g,j]
              mvSaved["pd2",1][i,g,j] <<- model[["pd2"]][i,g,j]
            }
          }
        }else{
          ...
          for(g in 1:n.year){
            for(j in 1:J[g]){
              model[["kern"]][i,g,j] <<- mvSaved["kern",1][i,g,j]
              model[["pd1"]][i,g,j] <<- mvSaved["pd1",1][i,g,j]
              model[["pd2"]][i,g,j] <<- mvSaved["pd2",1][i,g,j]
            }
          }
    ...
    for(i in 1:M){
      if(z.obs[i]==1&y2D[i,n.year]==0){ #for detected guys, skip if observed in final year
        ...
        for(g in (last.det):n.year){ #can't die on or before year of last detection
          ...
          model$N <<- N.curr - z.curr + z.prop
          model$calculate(ER.nodes) #update ER when N updated
          model$calculate(kern.nodes[i.idx]) #update kern nodes when pd changes
          model$calculate(pd1.nodes[i.idx]) #update kern nodes when pd changes
          model$calculate(pd2.nodes[i.idx]) #update kern nodes when pd changes
          # recruit likelihood conditional on having recruited (or alive in year 1)
          recruit.probs <- c(model$lambda.y1,model$ER)
          ...
          lp.y2 <- model$calculate(y2.nodes[i.idx])
          lp.surv <- model$calculate(z.nodes[i])
          lp.stop[g] <- lp.recruit + lp.y1 + lp.y2 + lp.surv
        }
        ...
        if(model$z.stop[i]!=z.stop.prop){#if proposal differs from current
          ...
          model$calculate(ER.nodes) #update ER when N updated
          model$calculate(kern.nodes[i.idx]) #update kern nodes when z changes
          model$calculate(pd1.nodes[i.idx]) #update pd nodes when z changes
          model$calculate(pd2.nodes[i.idx]) #update pd nodes when z changes
          #update these logProbs
          model$calculate(N.nodes[1])
          model$calculate(N.recruit.nodes)
          model$calculate(y1.nodes[i.idx])
    ...
    for(i in 1:M){
      if(z.obs[i]==0){
        ...
        lp.initial.y1 <- model$calculate(y1.nodes[i.idx])
        lp.initial.y2 <- model$calculate(y2.nodes[i.idx])
        ...
        model$calculate(ER.nodes) #update ER when N updated
        model$calculate(kern.nodes[i.idx]) #update kern nodes when z changes
        model$calculate(pd1.nodes[i.idx]) #update pd nodes when pd changes
        model$calculate(pd2.nodes[i.idx]) #update pd nodes when pd changes
        # recruit likelihood conditional on having recruited (or alive in year 1)
        recruit.probs <- c(model$lambda.y1,model$ER)
        recruit.probs <- recruit.probs/sum(recruit.probs)
        lp.proposed.recruit <- sum(log(recruit.probs[model$z.start])) #must consider all inds since ER can change. this is dcat()
        lp.proposed.y1 <- model$calculate(y1.nodes[i.idx])
        lp.proposed.y2 <- model$calculate(y2.nodes[i.idx])
        ...
        lp.initial.total <- lp.initial.recruit + lp.initial.y1 + lp.initial.y2 + lp.initial.surv
        lp.proposed.total <- lp.proposed.recruit + lp.proposed.y1 + lp.proposed.y2 + lp.proposed.surv
    ...
    for(up in 1:z.super.ups){ #how many updates per iteration?
      ...
      if(updown==0){#subtract
        ...
        if(!reject){
          ...
          lp.initial.y1 <- model$calculate(y1.nodes[pick.idx])
          lp.initial.y2 <- model$calculate(y2.nodes[pick.idx])
          ...
          model$N.survive <<- model$N[2:n.year]-model$N.recruit #survivors are guys alive in year g-1 minus recruits in this year g
          model$calculate(ER.nodes) #update ER when N updated
          model$calculate(kern.nodes[pick.idx]) #update kern nodes when z changes
          model$calculate(pd1.nodes[pick.idx]) #update pd nodes when pd changes
          model$calculate(pd2.nodes[pick.idx]) #update pd nodes when pd changes
          #get proposed logprobs for N and y
          lp.proposed.N <- model$calculate(N.nodes[1])
          lp.proposed.N.recruit <- model$calculate(N.recruit.nodes)
          lp.proposed.y1 <- model$calculate(y1.nodes[pick.idx]) #will always be 0
          lp.proposed.y2 <- model$calculate(y2.nodes[pick.idx]) #will always be 0

          lp.initial.total <- lp.initial.N + lp.initial.y1 + lp.initial.y2 + lp.initial.N.recruit
          lp.proposed.total <- lp.proposed.N + lp.proposed.y1 + lp.proposed.y2 + lp.proposed.N.recruit
          ...
          if(accept) {
            ...
            for(g in 1:n.year){
              for(j in 1:J[g]){
                mvSaved["kern",1][pick,g,j] <<- model[["kern"]][pick,g,j]
                mvSaved["pd1",1][pick,g,j] <<- model[["pd1"]][pick,g,j]
                mvSaved["pd2",1][pick,g,j] <<- model[["pd2"]][pick,g,j]
              }
            }
          }else{
            ...
            for(g in 1:n.year){
              for(j in 1:J[g]){
                model[["kern"]][pick,g,j] <<- mvSaved["kern",1][pick,g,j]
                model[["pd1"]][pick,g,j] <<- mvSaved["pd1",1][pick,g,j]
                model[["pd2"]][pick,g,j] <<- mvSaved["pd2",1][pick,g,j]
              }
            }
            #set these logProbs back
            model$calculate(N.nodes[1])
            model$calculate(N.recruit.nodes)
            model$calculate(y1.nodes[pick.idx])
            model$calculate(y2.nodes[pick.idx])
          }
        }
      }else{#add
        if(model$N.super[1] < M){ #cannot update if z.super maxed out. Need to raise M
          ...
          lp.initial.N <- model$getLogProb(N.nodes[1])
          lp.initial.N.recruit <- model$getLogProb(N.recruit.nodes)
          lp.initial.y1 <- model$calculate(y1.nodes[pick.idx]) #will always be 0
          lp.initial.y2 <- model$calculate(y2.nodes[pick.idx]) #will always be 0
