...
#I may or may not change the z update for z.super=1 individuals who were not captured. Currently proposing a sex,
...
source("Nimble Model JS-SCR_SexPopDy_mobileAC.R")
source("Nimble Functions JS-SCR_SexPopDy_mobileAC.R") #contains custom distributions and updates
source("sSampler Multi Sex.R") # activity center sampler that proposes from prior when z.super=0.
...
calcNodes <- c(N.nodes,N.recruit.nodes,
               ...
               y.nodes,z.nodes,phi.nodes,s.nodes) #the ones that need likelihoods updated in mvSaved
